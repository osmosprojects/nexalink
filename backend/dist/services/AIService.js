...
class AIService {
     ...
     */
    static async getConversationSuggestions(userId, contactId, customGoal) {
        ...
        const interactions = await InteractionRepository_1.InteractionRepository.list(userId, { contact_id: contactId, limit: 3 });
        const notes = await NoteRepository_1.NoteRepository.list(userId, { contact_id: contactId });
     ...
     */
    static async draftMessage(userId, params) {
        const contact = await ContactRepository_1.ContactRepository.getById(userId, params.contact_id);
        if (!contact)
            throw new Error('Contact not found');
     ...
     */
    static async summarizeMeeting(userId, params) {
     ...
     */
    static async getRelationshipInsights(userId) {
        const contacts = await ContactRepository_1.ContactRepository.list(userId, { limit: 100 });
        const insights = [];
        // Stale high-value contacts
        const staleHighValue = contacts.items.filter((c) => c.relationship_strength >= 70 &&
        ...
        // Overdue follow ups
        const overdueFollowUps = contacts.items.filter((c) => c.next_follow_up_at && new Date(c.next_follow_up_at).getTime() <= Date.now());
        ...
        // Goal momentum
        const goals = await GoalRepository_1.GoalRepository.list(userId, 'active');
        if (goals.length > 0) {
     ...
     */
    static async suggestGoals(userId) {
        const persona = await ProfileRepository_1.ProfileRepository.getPersonaByUserId(userId);
        return [
            {
