import { calculateMatchScore, generateAutoConnectRecommendations } from './AutoConnectEngine';

function runTests() {
  console.log('🧪 Running AutoConnectEngine Unit Tests...\n');

  // Test 1: Direct / Substring Match (95% Score)
  const score1 = calculateMatchScore('SaaS', 'SaaS', 'Acme', 'CTO', 'Alice');
  console.assert(score1 === 95, `Test 1 failed: expected 95, got ${score1}`);
  console.log(`✓ Test 1 Passed: Exact domain match score = ${score1}`);

  const score1b = calculateMatchScore('Enterprise SaaS Solutions', 'SaaS', 'Acme', 'CTO', 'Alice');
  ...
  const score2 = calculateMatchScore('Cloud Platforms & Infrastructure', 'Infrastructure', 'Nexa', 'Cloud Specialist', 'Bob');
  ...
  const score3 = calculateMatchScore('Fintech Banking Analytics', 'Payments', 'PayCo', 'Fintech Lead', 'Charlie');
  ...
  const score4 = calculateMatchScore('Healthcare BioTech', 'Automotive', 'AutoCorp', 'VP Manufacturing', 'David');
  ...
  const recs = generateAutoConnectRecommendations(userProfile, networkMembers);
