...
export function generateAutoConnectRecommendations(
  userProfile: NetworkProfileData,
  networkMembers: NetworkProfileData[]
  ...
  const myBridges: ConnectionBridge[] = Array.isArray(userProfile.connectionsOffered) ? userProfile.connectionsOffered : [];
  const currentUserName = userProfile.displayName || 'You';

  for (const member of networkMembers) {
    if (member.userId === userProfile.userId) continue;
    ...
    const memberBridges: ConnectionBridge[] = Array.isArray(member.connectionsOffered) ? member.connectionsOffered : [];
    ...
    for (const targetQuery of myTargets) {
      ...
      for (const bridge of memberBridges) {
        const score = calculateMatchScore(
          ...
          bridge.orgName || bridge.relationshipContext || bridge.relationship || '',
          bridge.role || '',
          bridge.personName || ''
        );
