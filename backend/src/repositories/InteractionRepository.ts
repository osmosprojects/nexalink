...
export class InteractionRepository {
  static async list(userId: number, filters: { contact_id?: number; goal_id?: number; type?: string; limit?: number; page?: number } = {}) {
    const page = Math.max(1, Number(filters.page) || 1);
    const limit = Math.min(100, Math.max(1, Number(filters.limit) || 20));
    const offset = (page - 1) * limit;

    const whereClauses: string[] = ['i.user_id = ?'];
    ...
    const countRows = await query<any[]>(
      `SELECT COUNT(*) as total FROM interactions i WHERE ${whereSql}`,
      params
    );
    const total = countRows[0]?.total || 0;

    const items = await query<InteractionRow[]>(
      `SELECT 
        i.*,
  ...
  static async getById(userId: number, interactionId: number): Promise<InteractionRow | null> {
      ...
      if (data.goal_id) {
        ...
        await conn.execute(
          `INSERT INTO goal_progress (goal_id, progress_date, progress_value, notes)
           VALUES (?, CURDATE(), 1, ?)`,
          [data.goal_id, `Progress logged from interaction: ${data.title}`]
        );
      }
  ...
  static async delete(userId: number, interactionId: number): Promise<boolean> {
