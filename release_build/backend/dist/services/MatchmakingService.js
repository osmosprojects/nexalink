...
const GoalTaxonomy_1 = require("../config/GoalTaxonomy");
class MatchmakingService {
    ...
    static matchGeography(userA, userB) {
    ...
    static matchIntents(goalsA = [], connectionsOfferedB = [], jobTitleB = '') {
        let points = 0;
        const reasons = [];
        const jobTitleLower = (jobTitleB || '').toLowerCase();
        for (const goalStr of goalsA) {
            const enumKey = goalStr;
            const complementRule = GoalTaxonomy_1.GOAL_COMPLEMENT_MAP[enumKey];
            if (!complementRule)
                continue;
            ...
            const matchedBridge = connectionsOfferedB.find((b) => complementRule.targetBridgeDomains.some((d) => (b.businessDomain || '').toLowerCase().includes(d.toLowerCase())));
            if (matchedBridge) {
                points += 15;
    ...
    static matchBridges(goalsA = [], targetBusinessesA = [], bridgesB = []) {
        if (!bridgesB.length)
    ...
    static async processProfileMatches(sourceUserId) {
        try {
         ...
         WHERE u.user_id != ? AND u.status = 'active'`, [sourceUserId]);
            ...
            await UserMatchScoreRepository_1.UserMatchScoreRepository.upsertScoreBatch(batchPrecomputations);
            return matchResults;
        }
        catch (err) {
    ...
    // ==========================================
    static async getDiscoverFeed(userId, limit = 10, offset = 0) {
        let topMatches = await UserMatchScoreRepository_1.UserMatchScoreRepository.getTopMatches(userId, limit, offset);
        // If no precomputed scores found, trigger calculation dynamically
        if (topMatches.length === 0) {
            await this.processProfileMatches(userId);
            topMatches = await UserMatchScoreRepository_1.UserMatchScoreRepository.getTopMatches(userId, limit, offset);
        }
        if (topMatches.length === 0)
            return [];
        ...
        for (let i = 4; i < finalResults.length; i += 5) {
            const diversityPick = await UserMatchScoreRepository_1.UserMatchScoreRepository.getRandomDiversityMatches(userId, excludeIds, 1);
