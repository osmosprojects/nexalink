...
async function runSeeds() {
    ...
    try {
        ...
        // 1. Create Demo User
        const [userRes] = await connection.execute(`INSERT INTO users (email, password_hash, display_name, avatar_url, status)
       VALUES (?, ?, ?, ?, ?)`, [
            'demo@nexalink.com',
            passwordHash,
            'Rahul Mehta',
        ...
        // 3. User Persona
        await connection.execute(`INSERT INTO user_personas (user_id, persona_name, communication_style, preferred_people, networking_goal, interests, confidence, is_active)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
            userId,
        ...
        for (const t of tagsData) {
            const [tRes] = await connection.execute(`INSERT INTO tags (user_id, name, color) VALUES (?, ?, ?)`, [userId, t.name, t.color]);
        ...
        for (const c of contactsData) {
            const [cRes] = await connection.execute(`INSERT INTO contacts (
          user_id, first_name, last_name, email, phone, company, job_title, location, linkedin_url, avatar_url, relationship_type, relationship_strength, last_interaction_at, next_follow_up_at, notes
            ...
            for (const tName of c.tags) {
                if (tagMap[tName]) {
                    await connection.execute(`INSERT IGNORE INTO contact_tags (contact_id, tag_id) VALUES (?, ?)`, [contactId, tagMap[tName]]);
                }
            }
        }
        // 6. Goals
        ...
        for (const g of goalsData) {
            const [gRes] = await connection.execute(`INSERT INTO goals (user_id, title, description, goal_type, target_value, current_value, unit, start_date, end_date, status)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [userId, g.title, g.description, g.goal_type, g.target_value, g.current_value, g.unit, g.start_date, g.end_date, g.status]);
            const gid = gRes.insertId;
            goalIds.push(gid);
            // Goal progress
            await connection.execute(`INSERT INTO goal_progress (goal_id, progress_date, progress_value, notes) VALUES (?, ?, ?, ?)`, [gid, '2026-09-10', 5, 'Great progress from SF Tech Week introductions.']);
            await connection.execute(`INSERT INTO goal_progress (goal_id, progress_date, progress_value, notes) VALUES (?, ?, ?, ?)`, [gid, '2026-09-15', 3, 'Added 3 founders from AI Demo Night.']);
        }
        // 7. Interactions
        ...
        for (const inter of interactionsData) {
            await connection.execute(`INSERT INTO interactions (
        ...
        for (const m of meetingsData) {
            await connection.execute(`INSERT INTO meetings (
        ...
        for (const t of tasksData) {
            await connection.execute(`INSERT INTO tasks (user_id, contact_id, goal_id, title, description, status, priority, due_date, sort_order)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [userId, t.contact_id, t.goal_id, t.title, t.description, t.status, t.priority, t.due_date, t.sort_order]);
        }
        // 10. Notes
        const notesData = [
            {
        ...
        for (const n of notesData) {
            await connection.execute(`INSERT INTO notes (user_id, contact_id, title, content, is_pinned) VALUES (?, ?, ?, ?, ?)`, [userId, n.contact_id, n.title, n.content, n.is_pinned]);
        }
        // 11. Recommendations
        ...
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [userId, r.recommended_name, r.recommended_role, r.recommended_company, r.avatar_url, r.industry, r.skills, r.reason, r.score, r.status]);
        }
        ...
        for (const p of postsData) {
            await connection.execute(`INSERT INTO posts (user_id, author_name, author_title, author_avatar, content, tags, likes_count)
         VALUES (?, ?, ?, ?, ?, ?, ?)`, [userId, p.author_name, p.author_title, p.author_avatar, p.content, p.tags, p.likes_count]);
        }
        // 13. Notifications
        ...
        for (const notif of notificationsData) {
            await connection.execute(`INSERT INTO notifications (user_id, type, title, message, entity_type, entity_id, is_read)
         VALUES (?, ?, ?, ?, ?, ?, ?)`, [userId, notif.type, notif.title, notif.message, notif.entity_type, notif.entity_id, notif.is_read]);
        }
        // 14. Audit Logs
        await connection.execute(`INSERT INTO audit_logs (user_id, action, entity_type, entity_id, metadata, ip_address, user_agent)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [
            userId,
